...
def train(args):
    ...
    st_val = SceneTextDataset('val', root_dir=dataset_config['root_dir']) #validation dataset
    ...
    for i in range(num_epochs):
        ...
        faster_rcnn_model.train()       #as it gets val below, ensure it is in train mode
        ...
        with torch.no_grad():
                for ims, targets, _ in tqdm(val_dataset):
                    ...
                    for target in targets:
                        ...
                        target['labels'] = target['labels'].long().to(device)
                    images = [im.float().to(device) for im in ims]
                    batch_losses = faster_rcnn_model(images, targets,epoch_num=i)
                    break
